...
export default function AusentismoTab({ rawData }: { rawData: any[][] }) {
  ...
  // Función para leer el número correctamente venga como venga
  ...
  // Función maestra: Busca palabra clave y extrae los números a su derecha
  ...
  const getEvolucionData = () => ({
    ...
    datasets: [
      {
        label: 'Ausentismo Real',
        data: realAcumulado,
        borderColor: COLORS.naranjo,
        backgroundColor: 'rgba(228, 83, 2, 0.2)',
        fill: true, tension: 0.4, borderWidth: 3, pointBackgroundColor: COLORS.naranjo
      },
      {
        label: 'Proyectado 2026 (Total)',
        data: proyectadoTotal,
        borderColor: COLORS.gris,
        borderDash: [5, 5],
        fill: false, tension: 0.4, borderWidth: 2, pointBackgroundColor: COLORS.gris
      }
    ]
  });
  ...
  const lineOptions: any = { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'bottom' }, datalabels: { align: 'top', color: COLORS.gris, font: { weight: 600 }, formatter: formatLabel } } };
  const stackedBarOptions: any = { responsive: true, maintainAspectRatio: false, scales: { x: { stacked: true }, y: { stacked: true } }, plugins: { legend: { position: 'bottom' }, datalabels: { color: COLORS.blanco, font: { weight: 600 }, formatter: formatLabel } } };
  const barOptions: any = { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false }, datalabels: { color: COLORS.blanco, anchor: 'end', align: 'start', font: { weight: 600 }, formatter: formatLabel } } };
  const doughnutOptions: any = { responsive: true, maintainAspectRatio: false, cutout: '65%', plugins: { legend: { position: 'bottom' }, datalabels: { color: COLORS.blanco, font: { weight: 600 }, formatter: formatLabel } } };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '25px', fontFamily: "'Poppins', sans-serif" }}>
      ...
      <div style={cardStyle}>
        ...
        <div style={{ height: '320px' }}><Line data={getEvolucionData()} options={lineOptions} /></div>
      </div>

      <div style={cardStyle}>
        <h4 style={chartTitleStyle}>Ausentismo Actual por Área (Composición)</h4>
        <div style={{ height: '300px' }}><Bar data={getAusentismoArea()} options={stackedBarOptions} /></div>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '20px' }}>
        <div style={cardStyle}>
          <h4 style={chartTitleStyle}>Sobretiempo por Área</h4>
          <div style={{ height: '280px' }}><Bar data={getSobretiempoArea()} options={barOptions} /></div>
        </div>
        <div style={cardStyle}>
          <h4 style={chartTitleStyle}>Tipo de Sobretiempo</h4>
          <div style={{ height: '280px' }}><Doughnut data={getTipoSobretiempo()} options={doughnutOptions} /></div>
        </div>
      </div>
...
const summaryCardStyle: React.CSSProperties = { flex: 1, backgroundColor: COLORS.blanco, padding: '25px 15px', borderRadius: '10px', boxShadow: '0 4px 10px rgba(0,0,0,0.04)', textAlign: 'center', display: 'flex', flexDirection: 'column', justifyContent: 'center', minHeight: '110px' };
